...
import { qrApi, LINEQRData, QRResponse } from '@/lib/api/qr';

const COUNTRIES = [
  { code: 'TH' as const, name: 'Thailand', flag: '🇹🇭' },
  { code: 'TW' as const, name: 'Taiwan', flag: '🇹🇼' },
  { code: 'JP' as const, name: 'Japan', flag: '🇯🇵' },
  { code: 'VN' as const, name: 'Vietnam', flag: '🇻🇳' },
];

export default function CreateLINEQRPage() {
  ...
  const [countryCode, setCountryCode] = useState<LINEQRData['country_code']>('TH');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data: LINEQRData = {
        phone: phone.trim(),
        country_code: countryCode || undefined,
      };

      const qrResult = await qrApi.createLINE(data);
      setResult(qrResult);
    } catch (err: any) {
  ...
  const handleDownload = () => {
    ...
    link.download = `line-qr-${result.id}.png`;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-black">LINE QR Code</h1>
                <p className="text-sm text-gray-600">
                  Create a QR code for LINE messaging (165M+ users across Asia-Pacific)
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-green-50 rounded-lg border border-green-200">
              <LucideIcons.MessageCircle className="w-5 h-5 text-green-600" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="text-lg font-bold mb-4 text-black">LINE Contact Details</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {COUNTRIES.map((country) => (
                      <button
                        ...
                        className={`
                          px-4 py-3 rounded-lg border-2 font-medium text-sm transition-all flex items-center gap-2
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Include country code ({selectedCountry?.flag} {selectedCountry?.name})
                  </p>
                </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-blue-700 text-sm">
                    Scanning this QR code opens LINE messenger and starts a conversation with
                    this phone number. LINE has over 165 million users across Thailand, Taiwan,
                    Japan, and Vietnam.
                  </p>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-green-600 text-sm mt-1">
                        Your LINE QR code is ready to use
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex items-start gap-2">
                      <LucideIcons.Globe className="w-4 h-4 text-gray-400 flex-shrink-0 mt-0.5" />
