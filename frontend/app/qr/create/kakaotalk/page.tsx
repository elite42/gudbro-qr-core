...
import { qrApi, KakaoTalkQRData, QRResponse } from '@/lib/api/qr';

export default function CreateKakaoTalkQRPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data: KakaoTalkQRData = {
        phone: phone.trim(),
      };

      const qrResult = await qrApi.createKakaoTalk(data);
      setResult(qrResult);
    } catch (err: any) {
  ...
  const handleDownload = () => {
    ...
    link.download = `kakaotalk-qr-${result.id}.png`;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-black">KakaoTalk QR Code</h1>
                <p className="text-sm text-gray-600">
                  Create a QR code for KakaoTalk messaging (47M users in South Korea)
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-yellow-50 rounded-lg border border-yellow-200">
              <LucideIcons.MessageCircle className="w-5 h-5 text-yellow-600" />
              <span className="text-sm font-medium text-yellow-700">Asia-Pacific</span>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h2 className="text-lg font-bold mb-4 text-black">KakaoTalk Contact Details</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Include country code (+82 for South Korea)
                  </p>
                </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-blue-700 text-sm">
                    Scanning this QR code opens KakaoTalk and starts a conversation with
                    this phone number. KakaoTalk is South Korea's dominant messaging app
                  ...
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-green-600 text-sm mt-1">
                        Your KakaoTalk QR code is ready to use
                      </p>
                    </div>
                  </div>
